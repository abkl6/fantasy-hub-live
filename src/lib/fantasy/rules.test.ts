...
describe("streaming kickers and defences", () => {
  ...
  it("caps kickers too, and leaves skill players alone", () => {
    expect(applyBidRules(book, { position: "K", bid: 20, budget: 100, weeksLeft: 8 }).bid).toBe(
      MIN_BID,
    );
    expect(applyBidRules(book, { position: "WR", bid: 20, budget: 100, weeksLeft: 8 }).bid).toBe(20);
  });
